...
template<typename T>
size_t decode(uint ndims[3], int3 stride, Word *stream, Word *side_channel, T *out, int param, uint chunk_size, zfp_mode mode, side_channel_type table_type)
{
  int d = 0;
  size_t out_size = 1;
  ...
  if(d == 3) {
    ...
    cuZFP::ConstantSetup::setup_3d();
    stream_bytes = cuZFP::decode3<T>(dims, s, stream, side_channel, out, param, chunk_size, mode, table_type);
  }
  else if(d == 2) {
    uint2 dims;
    dims.x = ndims[0];
    ...
    cuZFP::ConstantSetup::setup_2d();
    stream_bytes = cuZFP::decode2<T>(dims, s, stream, side_channel, out, param, chunk_size, mode, table_type);
  }
  else if(d == 1) {
    uint dim = ndims[0];
    int sx = stride.x;
    ...
    cuZFP::ConstantSetup::setup_1d();
    stream_bytes = cuZFP::decode1<T>(dim, sx, stream, side_channel, out, param, chunk_size, mode, table_type);
  }
...
Word *setup_device_side_channel(zfp_stream *stream, const size_t size)
{
  bool stream_device = cuZFP::is_gpu_ptr(stream->side_channel->side_channel_data);
  assert(sizeof(uint64) == sizeof(Word)); // "CUDA version currently only supports 64bit words");

  if(stream_device) {
    return (Word*) stream->side_channel->side_channel_data;
  }

  Word *d_side_channel = NULL;
  if(cudaMalloc(&d_side_channel, size) != cudaSuccess) {
    std::cerr<<"failed to allocate device memory for side_channel\n";
  }
  if(cudaMemcpy(d_side_channel, stream->side_channel->side_channel_data, size, cudaMemcpyHostToDevice) != cudaSuccess) {
    std::cerr<<"failed to copy stream from host to device\n";
  }
  return d_side_channel;
}
...
cuda_decompress(zfp_stream *stream, zfp_field *field)
{
  ...
  if (mode == zfp_mode_fixed_rate) {
    ...
    if (table_type == 1) {
    /* Check why 'offset' doesnt work and this has to be hardcoded as 1 */
      table_size = ((size_t)blocks + chunk_size - 1 ) / chunk_size * sizeof(Word);
    }
    else if (table_type == hybrid) {
      /* Hardcoded to partitions of 32 values with a size of 72 bytes, could be defines */
      size_t partitions = (blocks + (32 * chunk_size) - 1) / (32 * chunk_size);
      table_size = partitions * 72;    
    }
    else {
      std::cerr<<"Non-supported side channel information type for GPU. Use hybrid or offset \n";
      return;
    }
    d_side_channel = internal::setup_device_side_channel(stream, table_size);
  ...
  if(field->type == zfp_type_float) {
    float *data = (float*) d_data;
    internal::decode(dims, stride, d_stream, d_side_channel, data, param, chunk_size, mode, table_type);
    d_data = (void*) data;
  }
  else if(field->type == zfp_type_double) {
    double *data = (double*) d_data;
  ...
  size_t words_read = stream->size / sizeof(Word);
  if(d_side_channel) {
    cudaFree(d_side_channel);
  }
  stream->stream->bits = wsize;
  // set stream pointer to end of stream
