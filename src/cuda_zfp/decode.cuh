...
public:
  ...
  {
    /* Suggestion: possibly move the functionality of the constructor to a seek function */
...
void decode_ints_rate(BlockReader<Size> &reader, const int max_bits, Int *iblock)
{
  ...
  unsigned char *perm = get_perm<Size>();
#if (CUDART_VERSION < 8000)
    #pragma unroll
#else
    #pragma unroll Size
...
__device__ void zfp_decode(BlockReader<BlockSize> &reader, Scalar *fblock, const int param, const zfp_mode mode, const int dims)
{
  ...
  if(s_cont)
  {
    ...
    switch(mode) {
      case zfp_mode_fixed_rate:
        /* param contains maxbits */
        maxbits = param - (int)ebits;
        decode_ints_rate<Scalar, BlockSize, UInt, Int>(reader, maxbits, iblock);
        break;
      case zfp_mode_fixed_accuracy:
        /* param contains minexp */
        maxprec = MAX(emax - param + 2 * (dims + 1), 0);
        decode_ints_planes<Scalar, BlockSize, UInt, Int>(reader, maxprec, iblock);
        break;
      case zfp_mode_fixed_precision:
        /* param contains maxprec */
        decode_ints_planes<Scalar, BlockSize, UInt, Int>(reader, param, iblock);
        break;
    }
